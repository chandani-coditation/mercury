...
_metrics_store = {
    ...
    "retrieval_times": [],
    "by_retrieval_type": defaultdict(lambda: {
        "count": 0,
        "empty_count": 0,
        "avg_scores": {}
    }),
...
def record_retrieval(
    ...
    try:
        ...
        for result in results:
            ...
            result_service = result.get("metadata", {}).get("service") if isinstance(result.get("metadata"), dict) else None
            result_component = result.get("metadata", {}).get("component") if isinstance(result.get("metadata"), dict) else None
        ...
        if valid_scores > 0:
            ...
            _metrics_store["avg_rrf_score"] = (1 - alpha) * _metrics_store["avg_rrf_score"] + alpha * avg_rrf
            
            if total_vector > 0:
                avg_vector = total_vector / len(results)
                _metrics_store["avg_vector_score"] = (1 - alpha) * _metrics_store["avg_vector_score"] + alpha * avg_vector
            
            if total_fulltext > 0:
                avg_fulltext = total_fulltext / len(results)
                _metrics_store["avg_fulltext_score"] = (1 - alpha) * _metrics_store["avg_fulltext_score"] + alpha * avg_fulltext
...
def get_metrics() -> Dict:
    ...
    try:
        ...
        if metrics["retrieval_count"] > 0:
            metrics["avg_results_per_retrieval"] = metrics["total_results"] / metrics["retrieval_count"]
        else:
        ...
        if total_service_checks > 0:
            metrics["service_match_rate"] = metrics["service_match_count"] / total_service_checks * 100
        else:
            metrics["service_match_rate"] = 0.0
        
        total_component_checks = metrics["component_match_count"] + metrics["component_mismatch_count"]
        if total_component_checks > 0:
            metrics["component_match_rate"] = metrics["component_match_count"] / total_component_checks * 100
        else:
            metrics["component_match_rate"] = 0.0
        
        # Calculate average retrieval time
        if metrics["retrieval_times"]:
            metrics["avg_retrieval_time_ms"] = sum(metrics["retrieval_times"]) / len(metrics["retrieval_times"])
            metrics["p95_retrieval_time_ms"] = sorted(metrics["retrieval_times"])[int(len(metrics["retrieval_times"]) * 0.95)]
        else:
