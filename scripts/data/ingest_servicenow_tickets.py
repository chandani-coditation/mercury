...
def map_csv_row_to_incident(row: Dict, field_mappings: Dict, severity_mapping: Dict) -> IngestIncident:
    ...
    tags = {
        ...
        "category": category,
        "severity": severity,
        "assignment_group": row.get(mappings.get("assignment_group", {}).get("source_column", "assignment_group"), ""),
    ...
    metadata = {
        ...
        "sys_updated_on": row.get(mappings.get("sys_updated_on", {}).get("source_column", "sys_updated_on"), ""),
        "u_reopen_count": row.get(mappings.get("u_reopen_count", {}).get("source_column", "u_reopen_count"), ""),
    }
...
def ingest_incident(incident: IngestIncident, ingestion_url: str = INGESTION_SERVICE_URL) -> tuple[bool, Optional[str]]:
...
def ingest_csv_file(file_path: Path, field_mappings: Dict, severity_mapping: Dict, ingestion_url: str) -> tuple[int, int]:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 (row 1 is header)
                try:
                    ...
                    title_preview = (incident.title[:50] + "...") if len(incident.title) > 50 else incident.title
                    
                    # Print progress to console
                    print(f"  [{row_num-1}/{total_rows}] Ingesting ticket {incident_id}: {title_preview}")
                    logger.info(f"  [{row_num-1}/{total_rows}] Ingesting ticket {incident_id}: {title_preview}")
...
def main():
    ...
    try:
        ...
        severity_mapping = field_mappings_config.get("severity_mapping", {}).get("impact_urgency_to_severity", {})
    ...
    if args.file:
        ...
        success, errors = ingest_csv_file(file_path, servicenow_mappings, severity_mapping, args.ingestion_url)
        ...
        for csv_file in csv_files:
            success, errors = ingest_csv_file(csv_file, servicenow_mappings, severity_mapping, args.ingestion_url)
            total_success += success
            total_errors += errors
    ...
    if total_success > 0:
        ...
        try:
            ...
            # Count documents
            cur.execute("SELECT COUNT(*) as doc_count FROM documents WHERE doc_type = %s", ('incident',))
