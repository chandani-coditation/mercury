    ...
    try:
        ...
        if feedback.feedback_type == "resolution":
            ...
            raise ValidationError(
                "feedback_type must be 'triage' or 'resolution'"
            )
        ...
        if feedback.feedback_type == "resolution" and feedback.user_edited:
            try:
                ...
                incident_service.update_resolution(
                    ...
                    resolution_evidence=current_incident.get(
                        "resolution_evidence"
                    ),
                ...
                try:
                    ...
                    if verified_resolution != feedback.user_edited:
                        ...
                        logger.info(
                            f"Resolution update verified: incident_id={incident_id}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Could not verify resolution update: {str(e)}"
                    )
            except Exception as e:
                # Non-fatal; still return feedback stored
                logger.warning(
                    f"Failed to update resolution output after feedback: {str(e)}"
                )
        ...
        if feedback.feedback_type == "triage" and feedback.user_edited:
            try:
                ...
                if not is_valid:
                    ...
                    logger.error(
                        f"User-edited triage output validation failed: {error_msg}"
                    )
                ...
                logger.debug(
                    f"Original system_output keys: {sorted(original_keys)}"
                )
                logger.debug(
                    f"User edited keys: {sorted(set(feedback.user_edited.keys()))}"
                )
                ...
                if forbidden_found:
                    ...
                    logger.error(
                        f"User tried to add forbidden fields: {error_msg}"
                    )
                    raise ValidationError(error_msg)
                ...
                if extra_keys:
                    ...
                    logger.error(
                        f"User tried to add disallowed fields: {error_msg}"
                    )
                    raise ValidationError(error_msg)

                incident_service.update_triage_output(
                    incident_id, feedback.user_edited
                )
                logger.info(
                    f"Triage output updated via feedback: incident_id={incident_id}"
                )

                # Verify the update by fetching the incident again
                try:
                    updated_incident = incident_service.get_incident(incident_id)
                    verified_triage_output = updated_incident.get(
                        "triage_output"
                    )
                    ...
                    logger.warning(
                        f"Could not verify triage output update: {str(verify_err)}"
                    )
            except ValidationError:
                raise  # Re-raise validation errors
            except Exception as e:
                logger.error(
                    f"Failed to update triage_output after feedback: {str(e)}",
                    exc_info=True,
                )
                raise HTTPException(
                    status_code=400,
        ...
        if feedback.policy_band:
            ...
            if not triage_output:
                raise ValidationError(
                    "Cannot update policy without triage output"
                )
            ...
            original_policy_band = current_policy_decision.get(
                "original_policy_band"
            )
            ...
            try:
                incident_service.update_policy(
                    incident_id, feedback.policy_band, policy_decision
                )
                logger.info(
                ...
                try:
                    ...
                    verified_policy_decision = updated_incident.get(
                        "policy_decision"
                    )
                ...
                # Non-fatal; still return feedback stored
                logger.warning(
                    f"Failed to update policy after feedback: {str(e)}"
                )
            ...
            if feedback_before_policy:
                ...
                try:
                    incident_service.update_policy(
                        incident_id, policy_band, policy_decision
                    )
                except Exception as e:
                    # Non-fatal; still return feedback stored
                    logger.warning(
        ...
        logger.error(
            f"Unexpected error storing feedback: {str(e)}", exc_info=True
        )
