...
def simulate_robusta_flow(alert_data: dict, collect_feedback: bool = True):
    """
    ...
    try:
        ...
        canonical_alert = {
            "alert_id": alert_data.get(
                "fingerprint", f"alert-{datetime.now(timezone.utc).timestamp()}"
            ),
            "source": "prometheus",
        ...
        if collect_feedback:
            ...
            results["enrichments"].append(
                {"title": " Triage Feedback Collected", "content": "Feedback stored successfully"}
            )
