...
def validate_resolution_output(resolution_output: Dict, context_chunks: List[Dict] = None) -> Tuple[bool, List[str]]:
    ...
    require_rollback_for = [r.lower() for r in config.get("require_rollback_for_risk_levels", ["medium", "high"])]
    ...
    if risk_level_upper in require_rollback_for:
        if not rollback_plan:
            errors.append(f"rollback_plan REQUIRED for risk_level '{risk_level_upper}' (production safety)")
        else:
            ...
            if isinstance(rollback_plan, list):
                ...
                if not rollback_steps:
                    ...
                    if len(rollback_steps) < 1:
                        ...
                        errors.append(f"Too many rollback steps: {len(rollback_steps)} (max: {max_rollback})")
                ...
                if rollback_commands is not None:
                    if not isinstance(rollback_commands, dict):
                        ...
                        for step_idx, cmd_list in rollback_commands.items():
                            if not isinstance(cmd_list, list):
                                errors.append(f"rollback_plan.commands_by_step['{step_idx}'] must be a list")
                            else:
                                for cmd in cmd_list:
                                    if not isinstance(cmd, str):
                                        errors.append(f"Rollback command in commands_by_step['{step_idx}'] must be a string")
                ...
                if risk_level_upper == "high":
                    if not preconditions:
                        logger.warning("High-risk resolution missing rollback preconditions (strongly recommended)")
                    
                    triggers = rollback_plan.get("triggers")
                    if not triggers or not isinstance(triggers, list):
                        logger.warning("High-risk resolution missing rollback triggers (strongly recommended)")
                ...
                errors.append(f"rollback_plan must be a list or dict, got: {type(rollback_plan).__name__}")
    ...
    # Even for low-risk, if rollback_plan is provided, validate its structure
        ...
        if isinstance(rollback_plan, dict):
            ...
            errors.append(f"rollback_plan must be a list or dict or null, got: {type(rollback_plan).__name__}")
