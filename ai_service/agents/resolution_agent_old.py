...
def resolution_agent(triage_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Remove rollback from all recommendations and clean actions (no SQL, no commands)
    cleaned_recommendations = []
    for rec in final_recommendations:
        cleaned_rec = rec.copy()
        # Remove rollback field if present
        if "rollback" in cleaned_rec:
            del cleaned_rec["rollback"]
        # Ensure action is plain English (no SQL, no commands)
        if "action" in cleaned_rec:
            # Clean any SQL queries or commands that might have slipped through
            action = cleaned_rec["action"]
            # Remove SQL patterns
            import re
            action = re.sub(r'\b(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|EXEC|EXECUTE|USE|DECLARE)\s+[^.]*\.', '', action, flags=re.IGNORECASE)
            action = re.sub(r'```[\s\S]*?```', '', action)  # Remove code blocks
            action = re.sub(r'\$[^\s]+', '', action)  # Remove command-line patterns like $command
            cleaned_rec["action"] = action.strip()
    ...
    logger.info(
        f"Resolution agent completed: {len(cleaned_recommendations)} recommendations, "
        ...
        for llm_rec in llm_recommendations:
            ...
            if step_id and step_id in algo_lookup:
                if step_id not in seen_step_ids:
                    ...
                    # Remove rollback from recommendations (user doesn't want it)
                    if "rollback" in merged:
                        del merged["rollback"]
